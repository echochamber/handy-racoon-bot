import { finalInteraction, modalBooleanSelect, modalCharacterSelect, simpleMessage } from '@/commands/discordMessageUtil.js';
...
export const CREATE_ITEM_COMMAND: RESTPostAPIApplicationCommandsJSONBody = {
  name: 'create',
  ...
  res.send({
    ...
    data: {
      ...
      components: [
        ...
        modalBooleanSelect('Is Attuned?', modalFields.IS_ATTUNED)
      ],
    },
  });
}
...
export async function handleModalSubmission(req: Request, res: Response) {
  ...
  const isAttuned = fields.find((c: any) => c.custom_id === modalFields.IS_ATTUNED && c.type === ComponentType.StringSelect)?.values?.[0] == "True"
  var item: MagicItem = {
    name: name,
    description: desc,
    isAttuned: isAttuned,
  ...
  req.log.info("Item Created %s", JSON.stringify(createdItem))
  req.log.info(interaction)
  res.send(simpleMessage(`Magic item **${createdItem.name}** created.`))
}

export const createItem = {
  command: CREATE_ITEM_COMMAND,
