...
export async function attuneItem(db: Firestore, characterId: string, itemId: string, doLog?: boolean) {
  ...
  if (doLog) {
    logger.logger.info(`Added attuned item ${itemId} to character ${characterId}`);
  }
...
export async function unattuneItem(db: Firestore, characterId: string, itemId: string, doLog?: boolean) {
  ...
  if (doLog) {
    logger.logger.info(`Removed attuned item ${itemId} from character ${characterId}`);
  }
...
export async function update(
  ...
  if (doLog) {
    logger.logger.info(`Updated character ${characterId}: %s`, updates);
  }
...
export async function transferItem(
  ...
  if (doLog) {
    logger.logger.info(`Transferred item ${itemId} from ${fromCharacterId} to ${toCharacterId}`);
  }
