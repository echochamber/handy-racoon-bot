...
import { simpleErrorEphemeral, simpleMessage } from '@/commands/discordMessageUtil.js';
...
export async function handleModalSubmission(req: Request, res: Response) {
  ...
  if (!characterId) {
    res.send(simpleErrorEphemeral('No character selected. Please try again.'));
    return;
  }
  ...
  try {
    ...
    res.send(simpleMessage(`Description for "${character.name}" updated successfully.`));
    return;
  } catch (e) {
    res.send(simpleErrorEphemeral(`Failed to update description for "${character.name}".`));
    return;
  }
}
