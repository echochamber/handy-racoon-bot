import { finalInteraction, messageSelectEntity, simpleErrorEphemeral } from '@/commands/discordMessageUtil.js';
...
export async function handleToCharacterSelect(req: Request, res: Response) {
  ...
  finalInteraction(res, interaction, `Transferred "${item.name}" from ${fromCharacter.name} to ${toCharacter.name}.`, false);
}
