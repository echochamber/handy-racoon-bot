...
export async function create(db: Firestore, item: MagicItem, doLog?: boolean): Promise<MagicItem> {
  logger.logger.info("We are creating item %s", JSON.stringify(item))
  const col = db.collection("magicItems");
  const itemDoc = col.doc();
  if (item.ownerId && item.isAttuned) {
    characterDao.attuneItem(db, item.ownerId, itemDoc.id);
  }
  ...
  if (doLog) {
    logger.logger.info(`MagicItem created with ID: ${itemDoc.id}`);
